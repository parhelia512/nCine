...
class WorkerStatistics
{
  public:
	...
	void start(unsigned char threadId)
	{
		ASSERT(threadId < numThreads_);
		if (threadId < numThreads_)
		{
			TimestampStats &timestats = timestampStats_[threadId];
			timestats.startTimestamps[timestats.timestampIndex] = nc::TimeStamp::now();
		}
	}

	void end(unsigned char threadId)
	{
		ASSERT(threadId < numThreads_);
		if (threadId < numThreads_)
		{
	...
	void reset()
	{
		...
		for (unsigned int threadId = 0; threadId < numThreads_; threadId++)
		{
			TimestampStats &timestats = timestampStats_[threadId];
			...
			timeAverages_[threadId] = 0.0f;
			totalFinishedJobs_[threadId] = 0;
		}
	}

	unsigned char numThreads() const { return numThreads_; }
	const float *timeDurations(unsigned char threadId) const {return timestampStats_[threadId].durations; }
	...
	const nctl::Array<float> &calcAverages()
	{
		for (unsigned int threadId = 0; threadId < numThreads_; threadId++)
		{
			const TimestampStats &timestats = timestampStats_[threadId];
			timeAverages_[threadId] = 0.0f;
			...
			for (unsigned int ts = 0; ts < TimestampCount; ts++)
			{
				...
				{
					timeAverages_[threadId] += timestats.durations[ts];
					validTimestatCount++;
				}
			}
			timeAverages_[threadId] /= validTimestatCount;
		}
...
{
	workerStats.start(nc::IJobSystem::threadId());
	workerStats.end(nc::IJobSystem::threadId());
}
...
void MyEventHandler::onFrameStart()
{
	...
	if (showImGui)
	{
		...
		{
			ImGui::Text("Number of threads: %u (main is #%u)", jobSystem.numThreads(), jobSystem.threadId());
			...
			if (ImGui::TreeNode("Statistics"))
			{
				...
				for (unsigned int threadId = 0; threadId < workerStats.numThreads(); threadId++)
				{
					auxString.format("Timestamps for T#%u", threadId);
					ImGui::PlotHistogram(auxString.data(), workerStats.timeDurations(threadId), WorkerStatistics::TimestampCount, 0, nullptr, 0.0f, maxTimeScale);
				}

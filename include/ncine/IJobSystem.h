...
// If set to 1, the `JobId` will be a 32bit integer that encodes the thread id (5 bits) and the pool index (27 bits)
...
static const JobId NullJobId = uintptr_t(nullptr);
#endif
...
class IJobSystem
{
  public:
	...
	static inline unsigned char threadId() { return threadId_; }

  protected:
	/// Total number of threads executing jobs (workers + main)
	unsigned char numThreads_;

	/// The thread id (main one has always the last id)
	static thread_local unsigned char threadId_;
